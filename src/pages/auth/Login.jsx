...
function ResendTimer({ trigger, onResend }) {
  ...
  return (
    <div style={{textAlign:"center",marginTop:"10px"}}>
      {secs > 0
        ? <span style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",color:"#6b7688"}}>
            {t("loginPage.resendTimer.resendIn", {secs})}
          </span>
        : <button onClick={onResend} style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",
  ...
  return (
    <form onSubmit={handleSubmit} className="fade-up"
      ...
      <div>
        ...
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",color:"#94a3b8",margin:"5px 0 0"}}>
...
function EmailTab({ onSuccess, portal = "healthcare", agreed = false, agreedFacilitation = false, onSwitchToIdLogin }) {
  ...
  if (step === "otp") return (
    <form onSubmit={verifyOTP} className="fade-up"
      ...
      <div style={{background:"var(--wc-sage)",border:"1px solid #86efac",
        ...
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",color:"#15803d",fontWeight:"600"}}>{t("loginPage.emailTab.otpSentTo")}</p>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"14px",color:"#14532d",fontWeight:"700",marginTop:"2px"}}>{email}</p>
        {isNew && <p style={{fontFamily:"'Inter',sans-serif",fontSize:"12px",color:"#16a34a",marginTop:"4px"}}>{t("loginPage.emailTab.newAccountNote")}</p>}
      </div>
      <div>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",fontWeight:"600",color:"#374151",textAlign:"center",marginBottom:"12px"}}>{t("loginPage.emailTab.enterOtp")}</p>
        <OTPBoxes value={otp} onChange={setOtp} disabled={loading}/>
        {err && <p style={{fontFamily:"'Inter',sans-serif",color:"#ef4444",fontSize:"12px",marginTop:"8px",textAlign:"center"}}>⚠ {err}</p>}
      ...
      <button type="button" onClick={() => {setStep("email"); setOtp(""); setErr("");}}
        style={{background:"none",border:"none",color:"var(--wc-muted)",fontFamily:"'Inter',sans-serif",fontSize:"13px",cursor:"pointer",textAlign:"center"}}>
  ...
  return (
    <form onSubmit={sendOTP} className="fade-up"
      style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      <div>
        <label style={{display:"block",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"5px"}} htmlFor="auth-login-email-address">{t("loginPage.emailTab.emailLabel")}</label>
        ...
        {err && <p style={{fontFamily:"'Inter',sans-serif",color:"#ef4444",fontSize:"12px",marginTop:"4px"}}>⚠ {err}</p>}
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"12px",color:"#6b7688",marginTop:"5px"}}>{t("loginPage.emailTab.otpNote")}</p>
      </div>
...
function SMSTab({ onSuccess, portal = "healthcare", agreed = false, agreedFacilitation = false, onSwitchToIdLogin }) {
  ...
  if (step === "otp") return (
    <form onSubmit={verifyOTP} className="fade-up"
      ...
      <div style={{background:"var(--wc-sage)",border:"1px solid #86efac",borderRadius:"10px",padding:"13px",textAlign:"center"}}>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",color:"#15803d",fontWeight:"600"}}>{t("loginPage.smsTab.otpSentTo")}</p>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"14px",color:"#14532d",fontWeight:"700",marginTop:"2px"}}>{cc} {mobile}</p>
      </div>
      <div>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",fontWeight:"600",color:"#374151",textAlign:"center",marginBottom:"12px"}}>{t("loginPage.smsTab.enterOtp")}</p>
        <OTPBoxes value={otp} onChange={setOtp} disabled={loading}/>
        {err && <p style={{fontFamily:"'Inter',sans-serif",color:"#ef4444",fontSize:"12px",marginTop:"8px",textAlign:"center"}}>⚠ {err}</p>}
        <ResendTimer key={resendKey} trigger={resendKey} onResend={() => { setOtp(""); sendOTP(); }}/>
      </div>
  ...
  return (
    <form onSubmit={sendOTP} className="fade-up"
      style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      <div>
        <label style={{display:"block",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"5px"}} htmlFor="auth-login-mobile-number">{t("loginPage.smsTab.mobileLabel")}</label>
...
function TwoFactorStep({ preAuthToken, onSuccess, onBack }) {
  ...
  return (
    <form onSubmit={submit} className="fade-up" style={{display:"flex",flexDirection:"column",gap:"14px"}}>
      <div style={{background:"var(--wc-sage)",border:"1px solid #86efac",borderRadius:"10px",padding:"13px",textAlign:"center"}}>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"13px",color:"#15803d",fontWeight:"600",margin:0}}>
          🔐 Two-Factor Authentication
        </p>
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"12.5px",color:"#166534",margin:"4px 0 0"}}>
      ...
      {err && <p style={{fontFamily:"'Inter',sans-serif",color:"#ef4444",fontSize:"12px",margin:0,textAlign:"center"}}>⚠ {err}</p>}
...
function StaffTab({ onSuccess, initialType }) {
  ...
  return (
    <form onSubmit={handle} className="fade-up"
      ...
      <div>
        <label htmlFor="auth-login-staff-type" style={{display:"flex",alignItems:"center",gap:"6px",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"700",color:"#374151",marginBottom:"7px",textTransform:"uppercase",letterSpacing:"0.4px"}}>
      ...
      {[["email","email",t("loginPage.staffTab.email"),t("loginPage.staffTab.emailPlaceholder")],
        ...
        <div key={k}>
          <label style={{display:"block",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"5px"}} htmlFor={`auth-login-staff-${k}`}>{label}</label>
          <div style={{position:"relative"}}>
      ...
      {err && <p style={{fontFamily:"'Inter',sans-serif",color:"#ef4444",fontSize:"12px"}}> ⚠ {err}</p>}
      ...
      {type === "hospital" && (
        <div style={{background:"var(--wc-sage)",border:"1px solid #86efac",borderRadius:"10px",padding:"12px 14px"}}>
          <p style={{fontFamily:"'Inter',sans-serif",fontSize:"12px",color:"#15803d",margin:0,lineHeight:"1.6"}}>
...
function PatientIdLoginTab({ onSuccess, onSwitchToOTP }) {
  ...
  return (
    <form onSubmit={handle} className="fade-up" style={{display:"flex",flexDirection:"column",gap:"14px"}}>
      <div>
        <label style={{display:"block",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"5px"}} htmlFor="auth-login-patientid">
          Patient ID
        </label>
      ...
      <div>
        <label style={{display:"block",fontFamily:"'Inter',sans-serif",fontSize:"12px",fontWeight:"600",color:"#374151",marginBottom:"5px"}} htmlFor="auth-login-patientid-password">
          Password
        </label>
        <div style={{position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",fontSize:"12.5px",fontFamily:"'Inter',sans-serif"}}>
      ...
      <p style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",color:"#94a3b8",textAlign:"center",margin:0}}>
...
function PasswordRequiredNotice({ resetToken, patientId, onGoToPasswordLogin }) {
  ...
  if (done) return (
    <div className="fade-up" style={{textAlign:"center",padding:"10px 0"}}>
      ...
      <p style={{fontFamily:"'Inter',sans-serif",fontSize:"14px",color:"#374151",marginBottom:"18px"}}>
        Password updated. Log in with your Patient ID and new password.
      </p>
      <button onClick={onGoToPasswordLogin} style={{
        background:"linear-gradient(135deg,var(--wc-green),var(--wc-green-dark))",color:"#fff",fontFamily:"'Inter',sans-serif",
  ...
  return (
    <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:"14px"}}>
      <div style={{background:"#eff8ff",border:"1px solid #bae6fd",borderRadius:"10px",padding:"13px"}}>
        {patientId && (
          <>
            <p style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",color:"var(--wc-teal)",margin:0,textTransform:"uppercase",letterSpacing:".5px",fontWeight:700}}>
              Your Patient ID
            </p>
            <p style={{fontFamily:"'Inter',sans-serif",fontSize:"20px",color:"var(--wc-navy)",margin:"2px 0 8px",fontWeight:800,letterSpacing:".5px"}}>
              {patientId}
            </p>
          </>
        )}
        <p style={{fontFamily:"'Inter',sans-serif",fontSize:"12.5px",color:"var(--wc-teal)",margin:0}}>
      ...
      {!resetting ? (
        <>
          ...
          <button onClick={() => setResetting(true)} style={{
            background:"none",border:"1.5px solid var(--wc-border)",color:"#374151",fontFamily:"'Inter',sans-serif",
  ...
  return (
    <div className="lg" style={{
      ...
      <div className="lg-left" style={{flex:"0 0 44%",display:"flex",flexDirection:"column",justifyContent:"center",padding:"60px",color:"#fff",position:"relative",zIndex:1}}>
        ...
        <h1 style={{fontFamily:"'Manrope',sans-serif",fontSize:"clamp(30px,3.5vw,50px)",fontWeight:"700",lineHeight:"1.15",marginBottom:"18px"}}>
        ...
        <div style={{width:"100%",maxWidth:"410px",background:"#fff",borderRadius:"20px",boxShadow:"0 40px 80px rgba(0,0,0,0.45)",overflow:"hidden"}}>
          ...
          <div style={{padding:"26px 30px"}}>
            ...
            {!showStaff && (
              <div style={{marginBottom:"18px"}}>
                ...
                <div style={{background:"var(--wc-sage)",border:"1.5px solid #86efac",borderRadius:"10px",
                  padding:"10px 14px",fontFamily:"'Inter',sans-serif",fontSize:"12.5px",
            ...
            <div style={{background:"var(--wc-warm-white)",border:"1px solid var(--wc-border)",
              ...
              {!agreed && <p style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",
            ...
            {isPatientFlow && (
              <div style={{background:"var(--wc-warm-white)",border:"1px solid var(--wc-border)",
                ...
                {!agreedFacilitation && <p style={{fontFamily:"'Inter',sans-serif",fontSize:"11px",
                  color:"#b45309",margin:"8px 0 0 24px"}}>
                  {t("loginPage.main.consentRequired")}
                </p>}
              </div>
            ...
            <div style={{marginTop:"20px",paddingTop:"16px",borderTop:"1px solid #f1f5f9"}}>
              ...
              <button onClick={() => setShowStaff(!showStaff)}
                style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",
                  ...
                  fontFamily:"'Inter',sans-serif",fontWeight:"700",fontSize:"13px",textAlign:"center"}}>
                {showStaff ? "🩺" : "🏢"} {showStaff ? t("loginPage.main.staffToggleToPatient") : t("loginPage.main.staffToggleToStaff")}
              </button>
              <div style={{display:"flex",justifyContent:"center"}}>
                <Link to="/" style={{fontFamily:"'Inter',sans-serif",fontSize:"12px",color:"#6b7688",textDecoration:"none"}}>{t("loginPage.main.backToHome")}</Link>
              </div>
            </div>
          </div>
        </div>
