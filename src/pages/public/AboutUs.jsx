...
function SH({badge,title,sub,dark=false}){
  ...
  return(
    <div ref={ref} className={`reveal${vis?" in":""}`} style={{textAlign:"center",marginBottom:"48px"}}>
      <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:"700",color:dark?"var(--wc-green-pale)":"var(--wc-green)",letterSpacing:"2px",textTransform:"uppercase",marginBottom:"10px"}}>{badge}</p>
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(26px,3.5vw,42px)",fontWeight:"700",color:dark?"#fff":"var(--wc-navy)",margin:"0 0 12px",lineHeight:1.15}}>{title}</h2>
      {sub&&<p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"16px",color:dark?"rgba(255,255,255,.62)":"var(--wc-muted)",maxWidth:"520px",margin:"0 auto",lineHeight:1.75,fontWeight:"300"}}>{sub}</p>}
    </div>
  );
}
      ...
      <section style={{background:"linear-gradient(135deg,#071524,var(--wc-navy) 60%,#062818)",paddingTop:"40px",position:"relative",overflow:"hidden"}}>
        ...
        <W s={{padding:"52px 24px 80px"}}>
          <div style={{display:"flex",gap:"8px",alignItems:"center",marginBottom:"20px"}}>
            ...
            <span style={{color:"var(--wc-green-pale)",fontSize:"13px",fontFamily:"'DM Sans',sans-serif"}}>{t("aboutPage.breadcrumb")}</span>
          </div>
          <div className="au-grid" style={{display:"grid",gridTemplateColumns:"1.2fr 0.8fr",gap:"56px",alignItems:"center"}}>
            <div>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:"700",color:"var(--wc-green-pale)",letterSpacing:"2px",textTransform:"uppercase",marginBottom:"14px"}}>{t("aboutPage.eyebrowStory")}</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"13px"}}>
              {[["16+","statYearsActive"],["500+","statLivesTouched"],["50+","statHospitalPartners"],["18+","statSpecializations"]].map(([n,key])=>(
                <div key={key} style={{background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.10)",borderRadius:"13px",padding:"18px",textAlign:"center"}}>
                  <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"30px",fontWeight:"700",color:"var(--wc-green-lighter)",margin:0,lineHeight:1}}>{n}</p>
        ...
        <W>
          <div className="au-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"56px",alignItems:"center"}}>
            <div ref={s1} className={`reveal${v1?" in":""}`}>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:"700",color:"var(--wc-green)",letterSpacing:"2px",textTransform:"uppercase",marginBottom:"10px"}}>{t("aboutPage.whoWeAreEyebrow")}</p>
              <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(24px,3.5vw,40px)",fontWeight:"700",color:"var(--wc-navy)",margin:"0 0 20px",lineHeight:1.2}}>
              ...
              {(hospitalPortal
                ...
                <p key={i} style={{fontFamily:"'DM Sans',sans-serif",fontSize:"15px",color:"#475569",lineHeight:"1.78",borderLeft:`3px solid ${i===0?"var(--wc-green)":i===1?"#0e7490":"#7c3aed"}`,paddingLeft:"16px",marginBottom:"14px",fontWeight:"300"}}>{para}</p>
              ))}
            </div>
            <div ref={s2} className={`reveal${v2?" in":""}`}>
              <div style={{background:"linear-gradient(135deg,var(--wc-navy),#112d52)",borderRadius:"22px",padding:"36px",boxShadow:"0 20px 50px rgba(11,31,58,.22)"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"13px"}}>
                  <div style={{width:"46px",height:"46px",background:"linear-gradient(135deg,var(--wc-green),var(--wc-green-light))",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"20px",fontWeight:"700",color:"#fff",fontFamily:"'Cormorant Garamond',serif"}}>R</div>
                  <div>
                    <p style={{fontFamily:"'DM Sans',sans-serif",fontWeight:"700",color:"#fff",fontSize:"15px",margin:0}}>R.V. Raman</p>
                    <p style={{fontFamily:"'DM Sans',sans-serif",color:"var(--wc-green-pale)",fontSize:"12px",margin:0}}>{t("aboutPage.founderTitle")}</p>
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:"11px",marginTop:"20px",padding:"12px 14px",background:"rgba(4,120,87,.15)",border:"1px solid rgba(16,185,129,.22)",borderRadius:"10px"}}>
                  ...
                  <p style={{fontFamily:"'DM Sans',sans-serif",color:"var(--wc-green-pale)",fontSize:"12px",fontWeight:"600",margin:0}}>{t("aboutPage.euroCert")}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(260px,100%),1fr))",gap:"20px"}}>
              {HOSPITAL_SERVICE_IDS.map(({id,ic})=>(
                <div key={id} className="val-card" style={{background:"var(--wc-warm-white)",border:"1px solid var(--wc-border)",borderRadius:"14px",padding:"24px 20px",boxShadow:"0 2px 10px rgba(11,31,58,.05)"}}>
                  ...
                  <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:"13.5px",color:"var(--wc-muted)",lineHeight:"1.7",margin:0,fontWeight:"300"}}>{t(`aboutPage.hospServices.${id}.d`)}</p>
                </div>
              ))}
            </div>
          </W>
