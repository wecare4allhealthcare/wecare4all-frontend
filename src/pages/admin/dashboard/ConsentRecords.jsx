...
export default function ConsentRecords({ token }) {
  ...
  return (
    <div>
      <SectionHead title="Consent Records" count={summary?.total_patients ?? 0} />

      {summary && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))",
          gap: "12px", marginBottom: "20px" }}>
          <div style={{ background: "#fff", border: "1px solid #e2eaf4", borderRadius: "12px", padding: "14px 16px" }}>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "11px", color: "#6b7688", fontWeight: 600 }}>Total Patients</div>
          ...
          <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "12px", padding: "14px 16px" }}>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "11px", color: "#15803d", fontWeight: 600 }}>✅ Consented</div>
          ...
          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "12px", padding: "14px 16px" }}>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "11px", color: "#b91c1c", fontWeight: 600 }}>⚠️ Not Yet Consented</div>
      ...
      <form onSubmit={applyFilters} style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "16px" }}>
        ...
        <select value={status} onChange={e => { setStatus(e.target.value); load(1, search, e.target.value); }}
          ...
          <option value="not_consented">Not Consented Only</option>
        </select>
        <button type="submit" className="ad-btn">Search</button>
      </form>

      {loading ? <Spinner /> : records.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px", color: "#6b7688",
          fontFamily: "'DM Sans',sans-serif" }}>No matching records.</div>
      ) : (
        <>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "'DM Sans',sans-serif", fontSize: "13px" }}>
              <thead>
                <tr style={{ borderBottom: "2px solid #e2eaf4", textAlign: "left" }}>
                  <th style={{ padding: "10px 8px", color: "#6b7688", fontWeight: 600 }}>Name</th>
                  <th style={{ padding: "10px 8px", color: "#6b7688", fontWeight: 600 }}>Contact</th>
                  <th style={{ padding: "10px 8px", color: "#6b7688", fontWeight: 600 }}>Portal</th>
                  <th style={{ padding: "10px 8px", color: "#6b7688", fontWeight: 600 }}>Consent Status</th>
                  <th style={{ padding: "10px 8px", color: "#6b7688", fontWeight: 600 }}>Accepted On</th>
                </tr>
              </thead>
              <tbody>
                {records.map(r => (
                  <tr key={r.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    <td style={{ padding: "10px 8px", color: "#0b1f3a", fontWeight: 600 }}>{r.full_name || "—"}</td>
                    ...
                    <td style={{ padding: "10px 8px" }}>
                      {r.consent_accepted_at ? (
                        <span style={{ background: "#dcfce7", color: "#15803d", padding: "3px 10px",
                          borderRadius: "12px", fontSize: "11.5px", fontWeight: 700 }}>✅ Consented</span>
                      ) : (
                        <span style={{ background: "#fee2e2", color: "#991b1b", padding: "3px 10px",
                          borderRadius: "12px", fontSize: "11.5px", fontWeight: 700 }}>⚠️ Not Yet</span>
                      )}
                    </td>
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginTop: "18px" }}>
              <button disabled={page <= 1} onClick={() => load(page - 1, search, status)}
                className="btn-sm btn-outline" style={{ opacity: page <= 1 ? 0.5 : 1 }}>← Prev</button>
              ...
              <button disabled={page >= totalPages} onClick={() => load(page + 1, search, status)}
                className="btn-sm btn-outline" style={{ opacity: page >= totalPages ? 0.5 : 1 }}>Next →</button>
            </div>
          )}
        </>
      )}
